...
extension UIViewController {
    ...
    func wraningAlertDeletePV(titel: String?, message: String?, okButton: String, cancelButton: String , functionAction: @escaping () ->  Void ,functioncancel: @escaping () ->  Void ) {
        let alert = UIAlertController.init(title: titel, message: message, preferredStyle: .alert)
        let action = UIAlertAction.init(title: okButton, style: .default) { (action) in
            functionAction()
        }
    ...
    func warningPasswordChangeFailed(titel: String?, message: String?, okButton: String) {
    let alert = UIAlertController.init(title: titel, message: message, preferredStyle: .alert)
    let action = UIAlertAction.init(title: okButton, style: .cancel, handler: nil)
    alert.addAction(action)
    present(alert, animated: true, completion: nil)
    }
    func warningSetPassword(titel: String?, message: String?, okButton: String, functionAction: @escaping () ->  Void) {
        let alert = UIAlertController.init(title: titel, message: message, preferredStyle: .alert)
        let action = UIAlertAction.init(title: okButton, style: .default) { (action) in
            functionAction()
        }
